...
\begin{titlepage}
  ...
  \begin{minipage}{0.78\linewidth}
    \centering
    \normalsize
    Tema 1: Ondas electromagnéticas en el vacío\\
    Tema 2: Interacción radiación--materia\\
    Tema 3: Propagación en medios materiales\\
    Tema 4 y 4.2: Reflexión, refracción y anisotropía\\
    Tema 5: Difracción\\
    Tema 6: Interferencia
  \end{minipage}
...
materia. Aunque esta formulación es breve, engloba problemas de gran contenido
físico y matemático, y constituye un punto de encuentro privilegiado entre
modelización teórica, estructura geométrica y contraste experimental.

Según la escala espacial y el fenómeno de interés, se emplean tres niveles de
descripción jerárquicamente relacionados:

\begin{description}
  \item[\textbf{Óptica geométrica.}] Describe la luz como \emph{rayos} y usa el
        principio de Fermat; es adecuada cuando $\lambda \ll a$, de modo que
        difracción e interferencia quedan subdominantes.

  \item[\textbf{Óptica electromagnética (u ondulatoria).}] Describe la luz como
        \emph{campo electromagnético} que satisface Maxwell; es imprescindible
        cuando $\lambda \sim a$, y explica de forma natural difracción,
        interferencia y polarización.

  \item[\textbf{Óptica cuántica.}] Describe la luz y la materia mediante
        \emph{variables cuánticas} cuando el marco clásico no basta (fotones,
        emisión espontánea, estadística cuántica, estados no clásicos).
\end{description}

\begin{observacion}
Estas hipótesis no constituyen una deducción microscópica exacta, sino un
cierre fenomenológico lineal (restauración + disipación + forzamiento) que
captura con gran precisión la respuesta óptica fuera de estructuras cuánticas
finas.
\end{observacion}
...
cuántica. En este tema adoptaremos la descripción electromagnética; por tanto,
el punto de partida formal serán las ecuaciones de Maxwell y sus consecuencias
ondulatorias.
...
\begin{itemize}
  \item \eqref{eq:maxwell1} (\textbf{Gauss eléctrica}): la divergencia de
        $\ve{E}$ queda determinada por la densidad de carga local.
  \item \eqref{eq:maxwell2} (\textbf{Gauss magnética}): el campo magnético no
        presenta fuentes escalares aisladas (no hay monopolos magnéticos
...
Además, el vacío es el primer laboratorio teórico natural: elimina relaciones
constitutivas materiales y permite aislar la estructura dinámica pura del
...
\]
El argumento para $\ve{B}$ no es una mera repetición formal: refleja la
simetría estructural del acoplamiento dinámico entre ambos campos en Maxwell.
\end{proof}
...
luz y campo electromagnético. En términos de estructura teórica, este resultado
puede leerse como un corolario fundamental de Maxwell: la luz no se \emph{añade}
a la teoría, emerge de ella.
\end{observacion}
...
ancho espectral nulo). En la práctica toda fuente posee banda finita y tiempo
de coherencia finito; aun así, la aproximación armónica es extremadamente útil
cuando dicho ancho espectral es pequeño frente a las escalas características del
problema.
\end{definicion}
...
Fourier, señales de energía finita (o cuasi-monocromáticas en ventanas
temporales) pueden descomponerse en superposición de componentes
monocromáticas. Por linealidad, resolver el problema armónico
...
\begin{observacion}
El fasor $\widetilde{E}_j(\ve{r})$ no es una magnitud directamente observable:
es una variable auxiliar que codifica amplitud y fase de forma compacta. Las
magnitudes medidas experimentalmente se obtienen siempre a partir del campo real
o de promedios temporales del mismo.
\end{observacion}
...
\begin{itemize}
  \item[\checkmark] Es exacta para operadores lineales: suma, derivación,
        integración y ecuaciones lineales con coeficientes reales.
  ...
  \item[$\boldsymbol{\times}$] No puede aplicarse de forma ingenua en magnitudes
        ...
        al campo real.
\end{itemize}
\noindent El motivo algebraico es que la operación ``tomar parte real'' no
conmuta con productos cuadráticos: en general,
$\Re(z_1z_2^*)\neq \Re(z_1)\Re(z_2)$.
\end{proposicion}
...
\begin{observacion}
El paso conceptual ``frente plano $\Rightarrow$ onda plana'' es inmediato:
si las superficies de fase son planos paralelos y la amplitud es uniforme en
cada plano, la dependencia espacial solo puede entrar como $\ve{k}\cdot\ve{r}$.
\end{observacion}
\end{definicion}
...
sin contradecir la relatividad especial, porque la velocidad de señal está
ligada a la envolvente (velocidad de grupo y velocidad de frente), no a la
fase local.
\end{definicion}
...
\begin{observacion}
Convención de giro adoptada: mirando en la dirección de propagación, llamamos
dextrógiro al giro horario del extremo de $\ve{E}$ y levógiro al antihorario.
En la literatura pueden coexistir convenciones opuestas, por lo que es clave
fijarla explícitamente al comparar resultados.
\end{observacion}
...
\[
  ...
  = \frac{E_0^2}{2\mu_0 c}.
\]
\noindent El promedio elimina los términos oscilantes de frecuencia $2\omega$,
de modo que la intensidad resultante representa el transporte energético neto.
\end{proof}
...
Hasta aquí, el estudio ha sido puramente propagativo: hemos analizado la
dinámica del campo en vacío sin modelizar su acoplamiento con la materia. Ahora
...
1878), separando explícitamente hipótesis físicas y consecuencias observables.
...
\begin{observacion}
Esta aproximación fija el régimen de validez de la modelización microscópica
clásica: campo eléctrico dominante, velocidades no relativistas y correcciones
magnéticas de orden $v/c$ despreciables.
\end{observacion}
...
En lo que sigue se separan explícitamente tres niveles: hipótesis de
modelización (qué idealizaciones físicas se adoptan), resolución matemática
de la ecuación dinámica (qué se deduce del modelo) e interpretación física
...
La resolución de~\eqref{eq:lorentz_atom} se apoya en dos aproximaciones
controladas, cada una asociada a una escala física distinta:
...
La primera aproximación controla la variación espacial del campo en escala
atómica; la segunda controla la jerarquía de términos dinámicos en el régimen
no relativista. Juntas delimitan cuándo~\eqref{eq:lorentz_atom} describe
correctamente la respuesta del oscilador ligado.
...
\begin{observacion}
La potencia transferida cuantifica la energía extraída del campo incidente por
unidad de tiempo. En general, esta energía puede repartirse entre disipación
interna y reradiación. En forma de balance promedio:
\[
  \langle W\rangle = P_{\text{dis}} + P_{\text{esp}},
\]
donde $P_{\text{dis}}$ recoge pérdidas irreversibles internas y
$P_{\text{esp}}$ la energía reradiada hacia el exterior.
\end{observacion}
...
\begin{observacion}
Conviene distinguir potencia absorbida, potencia disipada y potencia esparcida.
En el límite lineal diluido tratado aquí, el balance energético se simplifica a
$\langle W\rangle = P_{\text{esp}}$: la energía extraída del campo incidente se
reradia casi por completo y la disipación interna es despreciable.
\end{observacion}
...
La dependencia $P_\text{esp} \propto 1/\lambda^4$ implica que las longitudes de
onda cortas (azul-violeta) se esparcen con mucha mayor eficacia que las largas
(rojo-naranja). Cuantitativamente, la razón de esparcimiento entre azul
($\lambda\approx450$ nm) y rojo ($\lambda\approx650$ nm) es
$(650/450)^4 \approx 4.4$.

\begin{description}
  \item[\textbf{Caso 1: Observador mirando al Sol directamente.}]
        La línea de visión atraviesa una columna atmosférica relativamente
        corta. El esparcimiento lateral elimina con mayor eficacia los
        componentes de longitud de onda corta, de modo que el haz directo
        queda relativamente enriquecido en componentes amarillas y rojas.
        Por ello, el disco solar se percibe \textbf{amarillo-blanco}.

  \item[\textbf{Caso 2: Observador mirando en una dirección oblicua.}]
        La luz que llega al observador es predominantemente luz
        \emph{esparcida} por moléculas atmosféricas. Como la eficiencia de
        esparcimiento crece al disminuir $\lambda$, la radiancia del cielo
        lateral está dominada por componentes cortas y el cielo aparece
        \textbf{azul}. El violeta contribuye menos a la percepción final por la
        menor sensibilidad visual del ojo humano en ese extremo espectral.

  \item[\textbf{Caso 3: Atardecer (luz rasante).}]
        En geometría rasante, la trayectoria óptica en atmósfera es
        considerablemente mayor que al mediodía. Tras ese recorrido, las
        componentes azules y violetas han sido fuertemente esparcidas fuera del
        haz directo, mientras que predominan las longitudes de onda largas.
        El resultado observado es una tonalidad \textbf{naranja-rojiza}.
\end{description}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[>=Stealth, scale=1.1]
    ...
    \node[align=left, anchor=west] at (4.6,1.1)
...
\begin{observacion}
Este efecto se mide de forma directa: el cielo lateral al Sol
(cerca de $90^{\circ}$) está polarizado y puede analizarse con un polarizador
lineal. En términos aplicados, esta estructura de polarización se usa en
navegación biológica y en teledetección atmosférica.
El grado de polarización es máximo cerca de $90^\circ$ en atmósfera limpia y
disminuye cuando aumenta la contribución de aerosoles (esparcimiento Mie).
\end{observacion}
...
\end{equation}
donde $\ve{D}$ es el desplazamiento eléctrico, $\ve{H}$ el campo magnético
auxiliar, $\ve{P}$ la polarización eléctrica y $\ve{M}$ la magnetización del
medio. Con estas definiciones,
...
En esta jerarquía, Maxwell aporta las \emph{leyes fundamentales} de balance y
acoplamiento, mientras que las relaciones de constitución aportan el
\emph{modelo material} (fenomenológico o microscópicamente derivado) que cierra
el sistema.
Este punto es clave: dos medios con las mismas ecuaciones de Maxwell pueden
mostrar propagación muy distinta si sus leyes constitutivas son diferentes.
...
\medskip
\begin{itemize}
  \item $\varepsilon_\text{gen}$ no depende de $\ve{r}$:
        \textbf{homogéneo}.
...
En un medio homogéneo ($\varepsilon_\text{gen}$ no depende de $\ve{r}$)
...
El paso de Maxwell temporal a Helmholtz armónica se apoya en una hipótesis
operativa precisa: dependencia temporal monocromática
$\propto e^{i\omega t}$. Bajo esta hipótesis, las derivadas temporales se
transforman en factores algebraicos y la propagación espacial queda codificada
en el problema elíptico~\eqref{eq:helmholtz}. En otras palabras, el problema se
separa en dos niveles: oscilación temporal fijada por $\omega$ y estructura
espacial gobernada por el operador de Helmholtz.
...
En esta descomposición, $\ve{k}$ controla la oscilación de fase y $\ve{a}$ la
atenuación espacial; de forma equivalente, en $n_c=\eta-i\kappa$, $\eta$
controla velocidad de fase y $\kappa$ absorción.
...
\end{mdframed}
\noindent
La primera igualdad fija el balance entre oscilación y atenuación; la segunda
cuantifica su acoplamiento geométrico cuando el medio es absorbente.
\end{proposicion}
...
\subsubsection{Clasificación de medios según \texorpdfstring{$\kappa$}{kappa}}
...
\subsubsection{Medios conductores \texorpdfstring{($\sigma\neq 0$)}{(sigma != 0)}}
...
\begin{equation}
  n_c^2 = 1 + \chi_e - i\frac{\omega_p^2}{\Gamma\omega + i\omega^2}
        = 1 + \chi_e
          - \frac{\omega_p^2}{\Gamma^2+\omega^2}
          - i\,\frac{\Gamma\,\omega_p^2}{\omega(\Gamma^2+\omega^2)}.
...
\begin{description}
  \item[\textbf{Caso 1: $\Gamma\ll\omega$ (poco rozamiento).}]
        $n_c^2 \approx 1+\chi_e - \omega_p^2/\omega^2
        - i\,\Gamma\omega_p^2/\omega^3$.
        \begin{itemize}
...
\begin{observacion}
La hipótesis de interfaz plana tiene alcance local general: toda superficie
suavemente curva puede aproximarse por su plano tangente en una vecindad
suficientemente pequeña. Por ello, las leyes deducidas aquí constituyen el
bloque elemental de la óptica de interfaces reales. Además, toda la teoría del
tema se obtiene de dos ideas estructurales: compatibilidad de fase sobre la
interfaz y condiciones de contorno de Maxwell para campos tangenciales.
\end{observacion}
...
\]
Como la interfaz es estacionaria, también se conserva la frecuencia:
$\omega_i=\omega_r=\omega_t$. Por ello, la interfaz conserva \emph{la
componente tangencial del vector de onda} y \emph{la frecuencia} de la onda.
...
\end{itemize}
Esta descomposición desacopla el problema en dos sistemas lineales
independientes ($p$ y $s$), cada uno gobernado por dos condiciones de contorno.
...
$s$ y $p$ explican la polarización parcial en reflexión oblicua. El signo de
$r_j$ codifica un posible salto de fase de $\pi$ en el campo reflejado, y por
tanto determina si la interferencia con otros haces será constructiva o
destructiva.
\end{observacion}
...
si aparece un segundo medio cercano (túnel óptico). Geométricamente, la fase se
transporta paralela a la interfaz mientras la amplitud decae en la normal.
...
Esta distinción amplitud--flujo es esencial: $|t_j|^2$ por sí solo no mide
energía transmitida salvo en geometrías muy particulares.
\end{nota}
...
En continuidad con el Tema~1, esto significa que la polarización no solo
describe la geometría del campo eléctrico: también selecciona velocidades de
fase distintas dentro del cristal.
...
Este formalismo unifica de manera compacta la descripción de polarización del
Tema~1 con los dispositivos anisótropos introducidos en este tema.
...
matemáticamente consistente. Por ello, cada resultado de este tema debe leerse
siempre junto con su régimen de validez.
...
\end{itemize}
Esta integral es el operador de propagación de referencia del tema; Fresnel y
Fraunhofer se obtienen como aproximaciones controladas de
\eqref{eq:hf_integral_t5}.
...
\begin{itemize}
  ...
  $z\gg a^2/\lambda$.
\end{itemize}
Físicamente, $N_F$ mide la importancia de la fase cuadrática a través de la
abertura: cuando $N_F\ll 1$ esa curvatura de fase es despreciable y la
descripción de Fourier (Fraunhofer) es consistente.
...
Desde el punto de vista metrológico, Young constituye el modelo básico para
extraer longitudes de onda a partir de una geometría interferencial simple.
...
Esa conversión directa fase$\to$intensidad explica su papel central en
...
En consecuencia, el dispositivo transforma una condición de fase en una
selectividad espectral extremadamente alta.
...
constante— es una idealización. Las fuentes reales emiten paquetes de onda de
