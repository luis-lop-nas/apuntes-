...
La óptica es la rama de la física que estudia la naturaleza y el comportamiento
de la luz: cómo se genera, cómo se propaga en el espacio y en la materia, y cómo
interacciona con los cuerpos materiales para producir fenómenos observables. A
pesar de la aparente sencillez de esta descripción, la física de la luz abarca
fenómenos de gran riqueza y profundidad, y su estudio ha sido históricamente
uno de los motores del desarrollo de la física teórica moderna.

En función de la escala espacial y del fenómeno considerado, se emplean tres
modelos de la luz jerárquicamente relacionados:

\begin{description}
  \item[\textbf{Óptica geométrica.}] Trata la luz como \emph{rayos rectos} que
        se propagan según el principio de Fermat del camino óptico mínimo. Es
        una aproximación válida cuando la longitud de onda $\lambda$ es mucho
        menor que las dimensiones características del sistema óptico
        ($\lambda \ll a$), de modo que los efectos ondulatorios pueden ignorarse.
        Es la base del diseño de instrumentos ópticos como lentes, telescopios
        y microscopios.

  \item[\textbf{Óptica electromagnética (u ondulatoria).}] Trata la luz como
        una \emph{onda electromagnética} descrita por las ecuaciones de Maxwell.
        Permite explicar fenómenos de carácter estrictamente ondulatorio como la
        difracción, la interferencia y la polarización, que escapan
        completamente a la descripción geométrica. Cuando $\lambda \sim a$, los
        efectos ondulatorios son ineludibles. Es la perspectiva adoptada en
        estos apuntes. Aplicaciones: láseres, espectroscopia, fibra óptica,
        óptica integrada.

  \item[\textbf{Óptica cuántica.}] Estudia la interacción luz-materia a nivel
        cuántico: los fotones son los cuantos del campo electromagnético, y la
        dualidad onda-partícula es inherente a la descripción. Esta capa
        teórica se vuelve imprescindible para explicar el efecto fotoeléctrico,
        la emisión espontánea y estimulada, y los estados no clásicos de la luz.
        Aplicaciones: computación cuántica, criptografía cuántica, metrología
        de precisión.
\end{description}

Estos tres modelos no son excluyentes: la óptica geométrica emerge como límite
de longitud de onda corta de la óptica electromagnética (límite $\lambda \to 0$),
y la óptica electromagnética emerge como límite clásico de la óptica cuántica.
En este tema nos centramos en la \textbf{óptica electromagnética}: la luz como
onda que satisface las ecuaciones de Maxwell en el vacío.
...
Cada ecuación tiene un significado físico preciso:
\begin{itemize}
  \item \eqref{eq:maxwell1}: ley de Gauss — las cargas son fuente del campo
        eléctrico.
  \item \eqref{eq:maxwell2}: inexistencia de monopolos magnéticos — las líneas
        de $\ve{B}$ son siempre cerradas.
  \item \eqref{eq:maxwell3}: ley de Faraday — un campo magnético variable
        induce un campo eléctrico rotacional.
  \item \eqref{eq:maxwell4}: ley de Ampère-Maxwell — las corrientes y el campo
        eléctrico variable son fuentes del campo magnético.
\end{itemize}
\end{definicion}
...
\begin{observacion}
Las cargas y corrientes variables en el tiempo generan campos $\ve{E}$ y $\ve{B}$;
a su vez, campos variables crean cargas efectivas y corrientes de desplazamiento.
Esta dependencia mutua es la responsable de la posibilidad de propagación de
perturbaciones electromagnéticas en ausencia de fuentes: la variación de
$\ve{B}$ genera $\ve{E}$ [ec.~\eqref{eq:maxwell3}], y la variación de $\ve{E}$
genera $\ve{B}$ [ec.~\eqref{eq:maxwell4}], cerrando el ciclo.
\end{observacion}
...
Dado que aquí nos interesa estudiar la \emph{propagación} de los campos
en el espacio libre —y no el problema de la generación— esta simplificación
es la adecuada. Las fuentes (antenas, átomos excitados, etc.) se sitúan
en una región acotada del espacio; lejos de ellas, los campos satisfacen
las ecuaciones homogéneas siguientes.
...
\begin{proof}
Aplicamos el operador $\curl$ a ambos miembros de la ecuación~\eqref{eq:vac3}:
\[
  \curl(\curl\ve{E}) + \curl\!\left(\pdv{\ve{B}}{t}\right) = \ve{0}.
\]
Dado que los operadores $\curl$ y $\partial/\partial t$ actúan sobre variables
independientes (espacio y tiempo, respectivamente), conmutan bajo las condiciones
de regularidad del campo:
\[
  \curl(\curl\ve{E}) + \pdv{}{t}(\curl\ve{B}) = \ve{0}.
\]
Aplicamos ahora la identidad vectorial del doble rotacional,
$\curl(\curl\ve{F}) = \grad(\div\ve{F}) - \nabla^2\ve{F}$, válida para cualquier
campo vectorial suficientemente regular:
\[
  \grad(\div\ve{E}) - \nabla^2\ve{E}
  + \pdv{}{t}(\curl\ve{B}) = \ve{0}.
\]
En el vacío, la ecuación~\eqref{eq:vac1} establece $\div\ve{E}=0$, por lo que
el primer término desaparece. Sustituyendo $\curl\ve{B}$ mediante la
ecuación~\eqref{eq:vac4}:
\[
  -\nabla^2\ve{E} + \mu_0\varepsilon_0\pdv[2]{\ve{E}}{t} = \ve{0}.
\]
Identificando la constante $c^2 = 1/(\mu_0\varepsilon_0)$, que tiene dimensiones
de velocidad al cuadrado, se obtiene:
\[
  \nabla^2\ve{E} - \frac{1}{c^2}\pdv[2]{\ve{E}}{t}= \ve{0}.
\]

Para $\ve{B}$ se procede de forma análoga pero partiendo de~\eqref{eq:vac4}:
\[
  \curl\ve{B} - \mu_0\varepsilon_0\pdv{\ve{E}}{t}= \ve{0}.
\]
Aplicando $\curl$ y usando la conmutatividad con $\partial/\partial t$:
\[
  \curl(\curl\ve{B}) - \mu_0\varepsilon_0\pdv{}{t}(\curl\ve{E})=\ve{0}.
\]
Aplicamos de nuevo la identidad del doble rotacional y sustituimos
$\curl\ve{E} = -\partial\ve{B}/\partial t$ [ec.~\eqref{eq:vac3}]:
\[
  \grad(\div\ve{B})-\nabla^2\ve{B}
  +\mu_0\varepsilon_0\pdv[2]{\ve{B}}{t}=\ve{0}.
\]
En el vacío, $\div\ve{B}=0$ por~\eqref{eq:vac2}, por lo que:
\[
...
\begin{observacion}
De este teorema se desprenden tres conclusiones fundamentales de alcance
histórico y físico:
\begin{enumerate}
  \item La luz es una onda que se propaga a partir de las fuentes que la generan.
  \item La luz puede propagarse en el vacío absoluto; no requiere ningún medio
        material de soporte (\emph{éter}), lo que resultó incompatible con la
        mecánica newtoniana clásica y motivó la relatividad especial.
  \item La luz es una onda \textbf{vectorial}: ambos campos $\ve{E}$ y $\ve{B}$
        son vectores, y su estado de vibración (polarización) es esencial para
        describir completamente la onda.
\end{enumerate}
La velocidad de propagación en el vacío, $c = 1/\sqrt{\mu_0\varepsilon_0}
\approx 2{,}998\times10^8\ \mathrm{m/s}$, se obtiene únicamente a partir de
las constantes eléctricas y magnéticas del vacío, lo que subraya la unificación
conceptual que implican las ecuaciones de Maxwell.
\end{observacion}
...
La onda se extiende en principio por todo el espacio y todo el tiempo; una
fuente monocromática perfecta sería por tanto idealizada. En la práctica,
todo láser o fuente de luz tiene un ancho espectral finito, pero la
aproximación monocromática es excelente en muchos contextos de óptica.
\end{definicion}
...
\end{equation}
Esta condición algebraica es la base del método de resolución de ecuaciones
diferenciales para campos armónicos: sustituyendo $\partial/\partial t \to i\omega$
(o $-i\omega$ según la convención de signo) las ecuaciones diferenciales parciales
se reducen a ecuaciones algebraicas en el dominio frecuencial.
\end{proposicion}
...
\begin{observacion}
Las ondas armónicas tienen una importancia que va más allá de su sencillez
analítica: por el \textbf{teorema de Fourier}, cualquier función de cuadrado
integrable puede descomponerse como superposición (integral) de funciones
armónicas de distintas frecuencias. Por tanto, las ondas armónicas forman una
\emph{base completa}: el estudio de la propagación de ondas monocromáticas
es suficiente para entender, mediante superposición, la propagación de
cualquier señal óptica arbitraria.
\end{observacion}
...
Por la fórmula de Euler, $e^{i\alpha} = \cos\alpha + i\sin\alpha$, cualquier
onda armónica real puede escribirse como la parte real de una exponencial
compleja. Para el campo eléctrico:
\begin{equation}
  E_j(\ve{r},t)
  ...
  \label{eq:compleja}
\end{equation}
donde $\widetilde{E}_j(\ve{r}) = A_j(\ve{r})\,e^{-i\delta_j(\ve{r})}$ es la
\emph{amplitud compleja} o \emph{fasorial}, que codifica simultáneamente la
amplitud real y la fase del campo en cada punto. En los cálculos se trabaja
con la amplitud compleja completa; la parte real se extrae solo al final para
obtener la magnitud física.
\end{definicion}
...
\begin{itemize}
  \item[\checkmark] La derivada e integral de una exponencial compleja sigue
        siendo una exponencial compleja (solo cambia la potencia de $i\omega$):
        $\partial/\partial t \mapsto i\omega$.
  \item[\checkmark] Es válida para cualquier operación \emph{lineal} (sumas,
        derivadas, integrales), gracias a la linealidad del operador parte real:
        \[
          \operatorname{Re}(A+B)=\operatorname{Re}(A)+\operatorname{Re}(B).
        \]
  \item[$\boldsymbol{\times}$] \textbf{No es válida} para operaciones no lineales
        como el producto —esencial en cantidades cuadráticas en el campo, como
        la intensidad o el vector de Poynting—:
        \[
          \operatorname{Re}(A\cdot B)\neq\operatorname{Re}(A)\cdot\operatorname{Re}(B).
        \]
        Para cantidades cuadráticas se debe volver a los campos reales, o bien
        utilizar la fórmula de promedio temporal para campos armónicos (véase la
...
$|\ve{k}\times\ve{E}| = k\,E\sin(90°) = kE$, luego:
\[
  \omega B = kE.
\]
...
La \emph{polarización} describe el estado de vibración del vector campo eléctrico
en una OAP: concretamente, la figura geométrica que traza el extremo de $\ve{E}$
en un plano transversal fijo a medida que transcurre el tiempo. Para propagación
en la dirección $z$, la transversalidad~\eqref{eq:transversal} impone $E_z=0$,
de modo que $\ve{E}$ está confinado al plano $XY$. Eligiendo convenientemente la
referencia de fase de modo que $\delta_x=0$ y definiendo $\delta=\delta_y-\delta_x$:
\begin{equation}
  E_x = E_{0x}\cos(\omega t - kz), \qquad
  E_y = E_{0y}\cos(\omega t - kz + \delta). \label{eq:polcomps}
\end{equation}
El estado de polarización queda completamente determinado por dos parámetros:
el cociente de amplitudes $E_{0y}/E_{0x}$ y la diferencia de fase $\delta \in
(-\pi, \pi]$.
\end{definicion}
...
radiación-materia propuesto por Hendrik Antoon Lorentz a finales del siglo XIX.
A pesar de su carácter clásico —los átomos son cuánticos—, captura correctamente
la física de la dispersión, la absorción y el índice de refracción en muchos
regímenes de interés práctico, y sus predicciones coinciden cualitativamente
con los resultados de la mecánica cuántica. Describe el átomo como un
\textbf{oscilador armónico amortiguado y forzado}.
...
Las ecuaciones de Maxwell son exactas a nivel microscópico, pero contienen las
contribuciones de cada carga individual, lo que las hace inmanejables para
describir un sólido con $\sim 10^{22}$ electrones/cm$^3$.
Para describir la materia ópticamente es necesario \textbf{promediar
espacialmente} sobre un volumen $\Delta V$ que satisfaga la doble condición:
\[
...
Un resultado fundamental es que las derivadas espaciales y el promedio
conmutan bajo estas condiciones:
\begin{equation}
