...
\begin{titlepage}
  ...
  {\Large\scshape Análisis Multivariante\par}
  \vspace{0.25cm}
  {\large Curso 2025--2026\par}

  \vspace{0.9cm}
  {\Huge\bfseries Tema 1\par}
  \vspace{0.4cm}
  {\large Introducción al Análisis Multivariante\par}

  \vspace{0.8cm}
  \rule{0.78\linewidth}{0.8pt}
  \vspace{0.6cm}
  {\normalsize
  Definición y tipos $\cdot$ Pasos del análisis $\cdot$ Formalización de los datos\\
  Elementos de álgebra lineal: vectores, matrices, autovalores, formas cuadráticas\par}
  \vspace{0.6cm}
  \rule{0.78\linewidth}{0.8pt}

  \vspace{1.4cm}
  % Diagrama decorativo: matriz esquemática
  \begin{tikzpicture}[>=Stealth, scale=1.0]
    \foreach \i in {0,1,2,3} {
      \foreach \j in {0,1,2} {
        \draw[line width=0.5pt] (\j*1.1, -\i*0.7) rectangle (\j*1.1+0.9, -\i*0.7+0.55);
      }
    }
    \node at (1.45, 0.75) {\small $\mat{X}_{n\times p}$};
    \node[anchor=west] at (3.7, -0.6) {\small $x_{ij}$: observación $i$, variable $j$};
    \draw[->] (3.3,-0.6) -- (3.6,-0.6);
  \end{tikzpicture}

  \vfill
  \begin{tabular}{@{}rl@{}}
    \textbf{Autor:}      & Luis López \\
    \textbf{Titulación:} & Grado en Estadística / Ciencia de Datos \\
...
Conjunto de técnicas estadísticas que permiten analizar \textbf{simultáneamente}
un conjunto de variables: varias medidas de cada elemento sometido a
investigación.
\end{definicion}

El elemento clave es el \textbf{valor teórico}: una combinación lineal de las
variables con ponderaciones determinadas empíricamente según el objetivo del
análisis.

\begin{mdframed}[style=resultado]
\[
  \text{Valor teórico} = w_1 X_1 + w_2 X_2 + \cdots + w_n X_n
\]
donde $X_i$ es la variable observada y $w_i$ la ponderación determinada por la
técnica multivariante elegida.
\end{mdframed}

El resultado es un valor único que representa la combinación de todo el conjunto
de variables que mejor se adaptan al objetivo específico del análisis.

\subsection{Objetivos principales}

El análisis multivariante persigue cuatro grandes objetivos:

\begin{enumerate}
  \item \textbf{Resumir} el conjunto de variables en unas pocas variables nuevas,
        construidas como transformaciones de las originales con la mínima pérdida
        de información.
        (\textit{ej.: componentes principales, análisis factorial}).

  \item \textbf{Encontrar grupos} en los datos, si existen.
        (\textit{ej.: tipología de procesos productivos, de clientes, de
        partículas}).

  \item \textbf{Clasificar} nuevas observaciones en grupos definidos.
        (\textit{ej.: clientes de un banco como fiables o no, personas como
        enfermas o no, partículas subatómicas como fermión o bosón}).

  \item \textbf{Relacionar} dos conjuntos de variables.
...
Existen diversas clasificaciones de las técnicas multivariantes. Seguimos la
propuesta de \textbf{Hair, Tathan y Black (1998)}, basada en el
\emph{tipo de relación entre variables}.
...
Existe al menos una variable dependiente que se quiere explicar a partir de
una o varias independientes.
...
No existe distinción entre variables dependientes e independientes: se analiza
la estructura interna del conjunto de datos.
...
Todo análisis multivariante sigue una secuencia de diez etapas:
...
Dado que disponemos de $n$ datos numéricos de una variable $X$, cada
observación se convierte en la componente $i$-ésima de un \textbf{vector
columna} en el espacio $\R^n$:
\[
...
Cuando medimos $p$ variables en cada uno de los $n$ elementos obtenemos:
\begin{itemize}
  \item Una \textbf{tabla} de $p$ columnas y $n$ filas:
        \begin{itemize}
          \item \emph{Columna}: valores de una variable.
          \item \emph{Fila}: valores de las $p$ variables de cada elemento.
        \end{itemize}
  \item Cada una de esas $p$ variables es una \textbf{variable escalar o
        univariante}; el conjunto de las $p$ variables forma una \textbf{variable
        vectorial o multivariante}.
\end{itemize}
...
[\mathbf{x}_{ij}]$, donde cada $\mat{X}_{(j)}$ es la $j$-ésima columna
(vector columna de $n$ filas y 1 columna) y cada fila es un vector fila de 1
fila y $p$ columnas.
\end{definicion}
...
\]
En particular, $|\mathbf{x}\mathbf{x}| = \mathbf{x}'\mathbf{x} = \sum_i x_i^2$.
\end{definicion}
...
\begin{itemize}
  \item \textbf{Matriz cuadrada de orden $n\times p$:} cumple $n = p$; en
        caso contrario se denomina \emph{rectangular}.

  \item \textbf{Matriz traspuesta} de $\mat{A}$ de orden $n\times p$: resulta
        de intercambiar filas por columnas, obteniendo una matriz de orden
        $p\times n$ que se denota $\mat{A}'$.

  \item \textbf{Traza} de una matriz cuadrada: suma de los elementos de la
        diagonal principal,
        \[
          \tr(\mat{A}) = \sum_i a_{ii}, \qquad \tr(\mat{A}) = \tr(\mat{A}').
        \]

  \item \textbf{Matriz simétrica:} $\mat{A} = \mat{A}'$.

  \item \textbf{Matriz nula o $\mat{0}$:} todos sus elementos son 0.

  \item \textbf{Matriz diagonal:} todos los elementos salvo los de la diagonal
        principal son nulos.

  \item \textbf{Matriz escalar $\mat{K}$:} matriz diagonal cuyos elementos no
        nulos son todos iguales:
        \[
          \mat{K} = \begin{pmatrix} k & 0 & 0 \\ 0 & k & 0 \\ 0 & 0 & k \end{pmatrix}.
        \]

  \item \textbf{Matriz identidad $\mat{I}$:} matriz diagonal cuyos elementos de
        la diagonal principal son 1:
        \[
          \mat{I} = \begin{pmatrix} 1 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 1 \end{pmatrix}.
        \]
\end{itemize}
\end{definicion}
...
número de vectores linealmente independientes de $\mat{A}$.
\end{definicion}
...
\]
Esta es la \textbf{ecuación característica} de $\mat{A}$: un polinomio de
grado $p$ en $\lambda$ con $p$ raíces $(\lambda_1, \lambda_2, \dots, \lambda_p)$.
\end{definicion}

\begin{observacion}
Una matriz de orden $n$ siempre tiene $n$ valores propios (contando
repeticiones). En general tiene $h \le n$ valores propios distintos; si un
valor propio aparece repetido $r$ veces se dice que tiene \emph{multiplicidad $r$}.
\end{observacion}
...
homogéneo $(\mat{A}-\lambda\mat{I})\mathbf{x}=\mathbf{0}$, que tiene
soluciones no triviales precisamente cuando $\lambda$ es autovalor.

Hay tantos autovectores independientes de $\mat{A}$ como autovalores.
...
\begin{ejemplo}
$\mathbf{x}_1 = \begin{pmatrix}2\\3\end{pmatrix}$,
$\|\mathbf{x}_1\| = \sqrt{4+9}=\sqrt{13}$:
\[
  \mathbf{u}_1 = \begin{pmatrix}2/\sqrt{13}\\3/\sqrt{13}\end{pmatrix}
  \approx \begin{pmatrix}0{,}55\\0{,}83\end{pmatrix}.
\]
$\mathbf{x}_2 = \begin{pmatrix}2\\-3\end{pmatrix}$,
$\|\mathbf{x}_2\|=\sqrt{13}$:
\[
  \mathbf{u}_2 = \begin{pmatrix}2/\sqrt{13}\\-3/\sqrt{13}\end{pmatrix}
  \approx \begin{pmatrix}0{,}55\\-0{,}83\end{pmatrix}.
\]
La \textbf{matriz de autovectores normalizados} es:
\[
...
\begin{center}
  \vspace*{0.4cm}
  {\Large\bfseries\scshape Tema 2}\\[0.3cm]
  {\large Distribuciones Multivariantes}\\[0.2cm]
  \rule{0.6\linewidth}{0.6pt}
\end{center}
\setcounter{section}{0}
\addtocontents{toc}{\protect\bigskip\protect\noindent\textbf{Tema 2: Distribuciones Multivariantes}\protect\medskip}
...
de $(x_1, x_2, \dots, x_p)$ es la \textbf{distribución de probabilidad
conjunta}. Dado un punto $\mathbf{x}^0 = (x_1^0, \dots, x_p^0)$, su función
de distribución conjunta $F(\mathbf{X})$ evaluada en $\mathbf{x}^0$ se define
como:
\begin{mdframed}[style=resultado]
\[
...
\]
\end{mdframed}
$P(\mathbf{x}\le\mathbf{x}^0)$ es la probabilidad de que la variable tome
valores menores o iguales al valor particular $\mathbf{x}^0$. La función de
distribución acumula las probabilidades de todos los valores menores o iguales
al punto considerado, y será no decreciente.
\end{definicion}

Habitualmente se trabaja con:
\begin{itemize}
  \item \textbf{Variable discreta:} función de probabilidad
        $p(\mathbf{x}^0) = P(\mathbf{x}=\mathbf{x}^0)
        = P(x_1=x_1^0, \dots, x_p=x_p^0)$.
  \item \textbf{Variable continua:} el vector $\mathbf{x}$ es absolutamente
        continuo si existe una función de densidad $f(\mathbf{x})$ tal que
        \[
...
Dada una variable aleatoria vectorial $p$-dimensional $(x_1,\dots,x_p)$,
...
\[
  f(x_1) = \int_{-\infty}^{+\infty} f(x_1,x_2)\,dx_2, \qquad
...
Si $\mathbf{x}_1$ y $\mathbf{x}_2$ son variables vectoriales, la
\textbf{distribución condicionada} de $\mathbf{x}_1$ dado un valor concreto
$\mathbf{x}_2 = \mathbf{x}_2^0$ se define como:
\[
  f(\mathbf{x}_1 | \mathbf{x}_2^0)
  = \frac{f(\mathbf{x}_1, \mathbf{x}_2^0)}{f(\mathbf{x}_2^0)},
  \qquad \text{supuesto } f(\mathbf{x}_2)\neq 0.
\]
\end{definicion}
...
\[
  f(\mathbf{x}_2|\mathbf{x}_1)
  = \frac{f(\mathbf{x}_1,\mathbf{x}_2)}{f(\mathbf{x}_1)} = f(\mathbf{x}_2),
\]
es decir, si $f(\mathbf{x}_1,\mathbf{x}_2) = f(\mathbf{x}_1)f(\mathbf{x}_2)$.
\end{definicion}

En general, las variables $\mathbf{x}_1, \dots, \mathbf{x}_p$ son
independientes si:
\[
  f(\mathbf{x}_1, \dots, \mathbf{x}_p)
  = f(\mathbf{x}_1)f(\mathbf{x}_2)\cdots f(\mathbf{x}_p).
\]
La independencia conjunta es una condición muy fuerte: cualquier subconjunto
$(x_1,\dots,x_h)$ con $h\le p$ también lo será, al igual que cualquier
conjunto de funciones $g_1(x_1)\cdots g_1(x_p)$.
...
\[
  ...
  E(\mathbf{X}) = \frac{1}{n}\sum_i \mathbf{x}_i, \qquad
  \boldsymbol{\mu} = \int \mathbf{X}\,f(\mathbf{X})\,d\mathbf{x}.
\]
El vector de medias muestral es $\bar{\mathbf{x}} = \dfrac{1}{n}\mathbf{1}'\mathbf{x}$.
\end{mdframed}
\end{definicion}

Propiedades del vector de medias:
\begin{itemize}
  \item Es la medida más utilizada: se encuentra en el centro de los datos,
...
\[
  \mat{S}_x = \frac{1}{n}
  \begin{pmatrix}
    (x_{i1}-\bar{x}_1)^2 & \cdots & (x_{i1}-\bar{x}_1)(x_{ip}-\bar{x}_p) \\
    \vdots & \ddots & \vdots \\
    (x_{ip}-\bar{x}_p)(x_{i1}-\bar{x}_1) & \cdots & (x_{ip}-\bar{x}_p)^2
  \end{pmatrix},
  \quad
  \mat{S} = \begin{pmatrix}
    S_1^2 & \cdots & S_{1p} \\ \vdots & \ddots & \vdots \\ S_{p1} & \cdots & S_p^2
  \end{pmatrix}.
\]
\end{definicion}
...
\[
  \tilde{\mathbf{X}} = \mathbf{X} - \mathbf{1}\cdot\bar{\mathbf{x}}
  = \mathbf{X} - \frac{1}{n}\mathbf{1}\cdot\mathbf{1}'\mathbf{x} = \mat{P}\mathbf{X},
\]
donde la \textbf{matriz de proyección} $\mat{P}$ es:
\[
...
Por tanto:
\[
  \mat{S} = \frac{1}{n}\tilde{\mathbf{X}}'\mathbf{X} = \frac{1}{n}\mathbf{X}'\mat{P}\mathbf{X}.
\]
...
\begin{proposicion}
La matriz $\mat{S}$ es semidefinida positiva: $\mathbf{y}'\mat{S}\mathbf{y}\ge0$
para todo vector $\mathbf{y}$.
\end{proposicion}

\begin{proof}
Sea $\mathbf{w}$ un vector cualquiera de dimensión $p$ y definamos la variable
escalar $v_i = \mathbf{w}'(\mathbf{x}_i-\bar{\mathbf{x}})$. Su media es
$\bar{v} = \frac{1}{n}\mathbf{w}'\sum(x_i-\bar{x}) = 0$ y su varianza:
\[
  \operatorname{var}(v) = \frac{1}{n}\sum_{i=1}^n v_i^2
  = \frac{1}{n}\sum_{i=1}^n [\mathbf{w}'(\mathbf{x}_i-\bar{\mathbf{x}})]
    [(\mathbf{x}_i-\bar{\mathbf{x}})'\mathbf{w}] = \mathbf{w}'\mat{S}\mathbf{w} \ge 0.
\]
Al ser válido para cualquier vector $\mathbf{w}$, $\mat{S}$ es semidefinida
positiva.
\end{proof}
...
Dados dos puntos $x_i, x_j\in\R^p$, una \textbf{distancia} es una función
$d(x_i,x_j)$ que satisface:
\begin{enumerate}
  \item $d(x_i,x_j)\ge0$ (no negativa).
  \item $d(x_i,x_i)=0$ (distancia de un elemento a sí mismo es 0).
  \item $d(x_i,x_j)=d(x_j,x_i)$ (simétrica).
  \item $d(x_i,x_j)\le d(x_i,x_p)+d(x_p,x_j)$ (desigualdad triangular).
\end{enumerate}
\end{definicion}

La distancia más utilizada es la \textbf{euclídea}, que depende de las
unidades de medida. También se definen la euclídea ponderada, la de Minkowski,
la de Mahalanobis y la distancia promedio.
...
\]
\end{mdframed}
Tiene en cuenta la variabilidad y las correlaciones entre las variables,
siendo invariante ante transformaciones lineales.
\end{definicion}
...
\[
  d_M^2 = \frac{1}{1-r^2}\left[
    \frac{(x_1-x_2)^2}{s_1^2} + \frac{(y_1-y_2)^2}{s_2^2}
    - 2r\frac{(x_1-x_2)(y_1-y_2)}{s_1s_2}
  \right].
\]
...
\[
  V_m = \frac{1}{n}\sum_{i=1}^n (x_i-\bar{x})'(x_i-\bar{x})
  = \sum_{i=1}^n \tr\!\left[\frac{1}{n}(x_i-\bar{x})(x_i-\bar{x})'\right]
  = \tr(\mat{S}).
\]
El promedio de distancias es la variabilidad total. Si se promedia también por
la dimensión del vector:
\[
  V_{m,p} = \frac{1}{np}\sum_{i=1}^n (x_i-\bar{x})'(x_i-\bar{x}) = \bar{s}^2.
\]
...
\begin{mdframed}[style=resultado]
\[
  \mat{R}_x = \mat{D}^{-\frac{1}{2}}\cdot\mat{S}_x\cdot\mat{D}^{\frac{1}{2}},
  \qquad \mat{D} = \operatorname{diag}(\sigma_1^2,\dots,\sigma_p^2).
\]
\end{mdframed}
\[
...
\begin{mdframed}[style=resultado]
\[
  r_{jk.12\dots p} = -\frac{s^{ij}}{\sqrt{s^{ii}s^{jj}}}.
\]
\end{mdframed}
\end{definicion}
...
intervalo:
\[
  f(x) = \frac{P(x_0 < X \le x_0+\Delta x)}{\Delta x}.
\]
...
que todas son diferenciables, la función de densidad de $\mathbf{y}$ es:
\begin{mdframed}[style=resultado]
\[
...
\begin{proof}
$\mat{S}_y = E[(\mathbf{y}-\boldsymbol{\mu}_y)(\mathbf{y}-\boldsymbol{\mu}_y)']
= E[\mat{A}(\mathbf{x}-\boldsymbol{\mu}_x)(\mathbf{x}-\boldsymbol{\mu}_x)'\mat{A}']
...
\]
\end{mdframed}
donde $\boldsymbol{\mu}\in\R^p$ es el vector de medias y $\boldsymbol{\Sigma}$
es la matriz de covarianzas (simétrica y definida positiva).
\end{definicion}
...
\begin{center}
  \vspace*{0.4cm}
  {\Large\bfseries\scshape Tema 3}\\[0.3cm]
  {\large Análisis de Componentes Principales}\\[0.2cm]
  \rule{0.6\linewidth}{0.6pt}
\end{center}
\setcounter{section}{0}
\addtocontents{toc}{\protect\bigskip\protect\noindent\textbf{Tema 3: Análisis de Componentes Principales}\protect\medskip}
...
\begin{enumerate}[label=\alph*)]
  ...
  \item $\operatorname{corr}(\mathbf{y}_i,\mathbf{y}_j)=0\ \forall i\neq j$,
        ...
        las matrices $\mat{S}$ y $\mat{R}$ son definidas positivas con
        $r(\mat{S})=r(\mat{R})=p$.
  \item $\displaystyle\sum_{i=1}^p\operatorname{var}(\mathbf{y}_i)=
        \sum_{i=1}^p\operatorname{var}(\mathbf{x}_i)$.
\end{enumerate}

Suponemos que hemos centrado las variables (media 0) $\Rightarrow$ la matriz
$\mathbf{X}$ tiene media 0 y matriz de covarianzas $\frac{1}{n}\mathbf{X}'\mathbf{X}$.
...
donde $r_i$ es la distancia entre el punto y su proyección. Sumando para
todos los puntos:
\[
...
Buscamos una combinación lineal $\mathbf{z}_1=\mathbf{X}\mathbf{a}_1$ de las
variables originales que tenga \textbf{varianza máxima}. Como $\mathbf{z}_1$
tiene media 0:
\[
...
\begin{proposicion}
En general, la matriz $\mathbf{X}$ (y por tanto $\mat{S}$) tiene rango $p$.
Existen tantas componentes principales como variables, que se obtienen
calculando los autovalores $\lambda_1\ge\lambda_2\ge\cdots\ge\lambda_p$ de
la ecuación característica:
\[
  |\mat{S}-\lambda\mat{I}|=0, \qquad (\mat{S}-\lambda_i\mat{I})\mathbf{a}_i=0.
\]
Los $\lambda_i$ son reales y positivos (al ser $\mat{S}$ simétrica y definida
positiva).
\end{proposicion}
...
\begin{enumerate}
  \item \textbf{Conservación de la variabilidad total.}
        ...
        \[
          \tr(\mat{S}) = \sum_{i=1}^p\operatorname{var}(x_i)
          = \lambda_1+\cdots+\lambda_p
          = \sum_{i=1}^p\operatorname{var}(z_i).
        \]
        Si llamamos $\mat{S}_x$ a la matriz de covarianzas de los componentes
        (diagonal con términos $\lambda_i$):
        \[
          |\mat{S}_x| = \lambda_1\cdots\lambda_p
          = \prod_{i=1}^p\operatorname{var}(z_i) = |\mat{S}_z|.
        \]
  ...
  \item \textbf{Covarianzas entre componentes y variables originales.}
        ...
        originales es:
        \[
          \operatorname{cov}(\mathbf{z},\mathbf{x}) = \frac{1}{n}\mat{Z}'\mathbf{X}
...
\begin{itemize}
  \item La proporción de varianza explicada por $\lambda_p^R$ es:
        $\dfrac{\lambda_p^R}{p}$.
...
\begin{itemize}
  ...
  \item Expresada en puntuaciones típicas, las saturaciones son las
        ...
        siendo $\mat{D}$ la matriz diagonal cuyos elementos son $1/s_i$.
\end{itemize}
...
\begin{enumerate}[label=\Roman*)]
  \item Cálculo de la matriz de covarianzas:
        $\mat{S} = \dfrac{\mathbf{x}'\mathbf{x}}{n-1}$.
  ...
  \item Cálculo de la matriz de puntuaciones de los sujetos en los
        componentes: $\mat{Y}=\mathbf{X}\mat{A}$.
...
\begin{enumerate}[label=\Roman*)]
  \item Cálculo de la matriz de correlaciones:
        $\mat{R} = \dfrac{\mathbf{z}'\mathbf{z}}{n-1}$.
  ...
  \item Cálculo de la matriz de puntuaciones típicas:
        $\mat{Z}_y=\mat{Z}_x\mat{A}=\mat{A}\boldsymbol{\Lambda}^{1/2}$.
  \item Cálculo de la varianza de cada componente:
...
\[
  \mat{A} = \begin{pmatrix}\cos\theta & -\operatorname{sen}\theta\\
             \operatorname{sen}\theta & \cos\theta\end{pmatrix}.
\]
...
\[
  ...
  \quad\cos\theta=0{,}722,\quad\operatorname{sen}\theta=0{,}692
