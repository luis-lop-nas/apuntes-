...
\begin{itemize}
  \item Los \textbf{escalares} se denotan en minúscula ($x,\lambda,r$), los
        \textbf{vectores} en negrita ($\mathbf{x},\boldsymbol{\mu}$) y las
        \textbf{matrices} en negrita mayúscula ($\mat{X},\mat{S},\mat{R}$).
  \item $\mat{X}\in\R^{n\times p}$ representa una matriz de datos con
        $n$ observaciones (filas) y $p$ variables (columnas).
  \item El superíndice prima ($'$) indica trasposición; $\mat{A}^{-1}$ indica
        matriz inversa cuando existe.
  ...
  \item $E(\mathbf{X})$, $\Var(\mathbf{X})$ y $\Cov(\mathbf{X},\mathbf{Y})$
        representan esperanza, varianza y covarianza.
  \item En contextos muestrales, se usará barra superior para medias
        ($\bar{\mathbf{x}}$) y letras con sombrero para estimadores
...
una combinación lineal de variables observadas (que aquí llamaremos
\textbf{valor teórico}), cuyas ponderaciones se estiman en función del objetivo
del análisis (síntesis, discriminación, predicción o descripción estructural).
...
El resultado es una variable escalar sintética que resume, en una dirección
determinada del espacio de variables, la información más relevante para el
problema estadístico planteado.

\subsection{Objetivos principales}

Desde el punto de vista aplicado, el análisis multivariante persigue cuatro
...
Existen varias taxonomías de técnicas multivariantes. Seguimos la propuesta de
\textbf{Hair, Tathan y Black (1998)}, basada en el \emph{tipo de relación
entre variables} y en la distinción entre estructura de dependencia e
interdependencia.
...
En este bloque se distingue una variable (o un conjunto de variables)
dependiente, cuyo comportamiento se modeliza a partir de una o varias variables
explicativas.
...
No se impone una separación previa entre variables respuesta y explicativas:
el interés se centra en la estructura interna del conjunto de variables y en
...
Cuando se miden $p$ variables en cada una de las $n$ unidades observacionales
se obtiene:
\begin{itemize}
  \item Una \textbf{tabla} de $p$ columnas y $n$ filas:
        ...
        \end{itemize}
  \item Cada una de esas $p$ variables es una \textbf{variable escalar}
        (univariante), mientras que su agregación define un \textbf{vector
        aleatorio} (variable multivariante).
\end{itemize}
...
Esta convención (filas = individuos, columnas = variables) se mantendrá en
todo el documento.
\end{definicion}
...
\begin{itemize}
  \item \textbf{Matriz cuadrada de orden $p$:} una matriz de tamaño
        $p\times p$.
        En caso $n\neq p$, la matriz es \emph{rectangular}.

  \item \textbf{Matriz traspuesta} de $\mat{A}\in\R^{n\times p}$: se obtiene
        intercambiando filas por columnas, y se denota
        $\mat{A}'\in\R^{p\times n}$.

  \item \textbf{Traza} de una matriz cuadrada: suma de los elementos de su
        diagonal principal,
        \[
...
En general, el número de autovectores linealmente independientes puede ser
menor que el número total de autovalores (contando multiplicidades). En
particular, si $\mat{A}$ es simétrica, existe una base ortonormal completa de
autovectores.
...
$\bar{v} = \frac{1}{n}\mathbf{w}'\sum_{i=1}^n(\mathbf{x}_i-\bar{\mathbf{x}}) = 0$
y su varianza:
\[
