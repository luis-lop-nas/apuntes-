...
\newcommand{\iniciotema}[3]{%
  \newpage
  \begin{center}
    {\Large\bfseries\scshape Tema #1}\\[0.3cm]
    {\large #2}\\[0.2cm]
...
\begin{titlepage}
  ...
  {\Large Apuntes de Estadística\par}
  \vspace{0.4cm}
  ...
  {\normalsize
  Temas 1--4: introducción, distribuciones multivariantes, análisis de componentes principales
  y análisis discriminante.\par}
...
\begin{variablesprimera}
  ...
  \varitem{F(\mathbf{x}^0)}{función de distribución conjunta acumulada}
...
\]
\begin{variablesprimera}
  \varitem{\mathbf{x}_1}{subvector de variables de interés}
  \varitem{\mathbf{x}_2}{subvector sobre el que se condiciona}
  \varitem{\mathbf{x}_2^0}{valor fijado del subvector $\mathbf{x}_2$}
  \varitem{f(\mathbf{x}_1 \mid \mathbf{x}_2^0)}{densidad condicionada de $\mathbf{x}_1$ dado $\mathbf{x}_2=\mathbf{x}_2^0$}
\end{variablesprimera}
\end{definicion}
...
Regla bayesiana básica: asignar $\mathbf{x}_0$ al grupo con mayor
probabilidad a posteriori; equivalentemente, al grupo que maximiza
$\pi_i f_i(\mathbf{x}_0)$.
...
La regla de decisión es lineal en $\mathbf{x}$ usando las funciones:
\begin{mdframed}[style=resultado]
\[
  \delta_i(\mathbf{x})=
...
\]
Como $D_A^2<D_B^2$, se clasifica en el grupo $A$.
...
Fisher plantea hallar una proyección escalar $z=\boldsymbol{\alpha}'\mathbf{x}$
que maximice la separación entre medias de grupo relativa a su dispersión
interna.
...
\[
  r_{jk.\,\text{resto}}=-\frac{s^{ij}}{\sqrt{s^{ii}s^{jj}}}.
\]
